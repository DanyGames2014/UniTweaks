...
public class KeyPressedListener {
    ...
    public static void keyPress() {
        ...
        if (Keyboard.getEventKeyState() && minecraft.currentScreen == null) {
            // Photo Mode
            if (isKeyDown(KeyBindingListener.photoMode.code)) {
                minecraft.setScreen(new PhotoModeScreen(null));
            }

            // Panorama Screenshot
            if (isKeyDown(KeyBindingListener.panoramaScreenshot.code)) {
                panoramaScreenshot();
            }

            // Dismount
            if (isKeyDown(KeyBindingListener.dismount.code)) {
                dismount();
            }

            // Rescan Mouse
            if (isKeyDown(KeyBindingListener.rescanMouse.code)) {
                RawInputHandler.getMouse("Player Triggered Rescan");
                Util.notify("Rescanning for Mice", true);
            }

            // Toggle Raw Input
            if (isKeyDown(KeyBindingListener.toggleRawInput.code)) {
    ...
    public static void handleHotbarKey() {
        int eventKey = Keyboard.getEventKey();
        if (eventKey == KeyBindingListener.hotbar1.code) {
            minecraft.player.inventory.selectedSlot = 0;
        } else if (eventKey == KeyBindingListener.hotbar2.code) {
            minecraft.player.inventory.selectedSlot = 1;
        } else if (eventKey == KeyBindingListener.hotbar3.code) {
            minecraft.player.inventory.selectedSlot = 2;
        } else if (eventKey == KeyBindingListener.hotbar4.code) {
            minecraft.player.inventory.selectedSlot = 3;
        } else if (eventKey == KeyBindingListener.hotbar5.code) {
            minecraft.player.inventory.selectedSlot = 4;
        } else if (eventKey == KeyBindingListener.hotbar6.code) {
            minecraft.player.inventory.selectedSlot = 5;
        } else if (eventKey == KeyBindingListener.hotbar7.code) {
            minecraft.player.inventory.selectedSlot = 6;
        } else if (eventKey == KeyBindingListener.hotbar8.code) {
            minecraft.player.inventory.selectedSlot = 7;
        } else if (eventKey == KeyBindingListener.hotbar9.code) {
